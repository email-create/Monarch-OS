...
export default function MonarchOS() {
  ...
  const cmTimerRef=useRef(null);const transcriptRef=useRef("");const lastSentRef=useRef("");const recognitionRef=useRef(null);const aiIntervalRef=useRef(null);const lowConfCount=useRef(0);const noSpeechTimer=useRef(null);
  ...
  // Call Mode timer (parent level, survives remounts)
  ...
  // Speech recognition + AI coaching (parent level)
  useEffect(()=>{
    ...
    try{
      ...
      aiIntervalRef.current=setInterval(async()=>{
        ...
        const cmSellerCalls=cmSeller?calls.filter(c=>c.leadId===cmSeller.id):[];
        try{
          ...
          if(parsed.trigger&&parsed.confidence>0.5){setWhispers(p=>{const next=[{id:Date.now().toString(36),trigger:parsed.trigger,coaching:parsed.coaching,ts:Date.now(),pinned:false},...p];return next.slice(0,4);});}
  ...
  useEffect(() => {
    const unsubs = [
      ...
      subscribe(sPath("aiCache"), (data) => { setAiCache(data || {}); }),
    ];
...
useEffect(() => { if(!skipSave.current && initialized.current.shared) saveData(sPath("buyers"), buyers); }, [buyers]);
  ...
  const CallsPage = () => {
    ...
    // ── Call Mode (state hoisted to parent) ──
    const fmtTimer=(s)=>`${String(Math.floor(s/60)).padStart(2,"0")}:${String(s%60).padStart(2,"0")}`;
    const exitCallMode=()=>{setCallMode(false);setCmStep("pick");setCmSeller(null);setCmDevice(null);setCmStage("opening");setCmLastStatus("");setCmSearch("");setWhispers([]);setMicStatus("off");transcriptRef.current="";lastSentRef.current="";};
  ...
  const AnalyzerPage = () => {
    ...
    const cacheKey=(addr)=>addr.trim().toLowerCase().replace(/[^a-z0-9]/g,"_").slice(0,80);
    ...
    const runAiEstimate=async(forceRefresh=false)=>{
      if(!f.address.trim()){addToast("Enter an address first","error");return;}
      const key=cacheKey(f.address);
      // Check cache first
      if(!forceRefresh&&aiCache[key]){
        applyAiResult(aiCache[key].result);setAiFromCache(true);
        addToast("Loaded from cache (free)");return;
      }
      setAiLoading(true);setAiError("");setAiResult(null);setAiFromCache(false);
      try{
        ...
        // Save to cache
        const updated={...aiCache,[key]:{result:parsed,address:f.address.trim(),date:new Date().toISOString()}};
        setAiCache(updated);saveData(sPath("aiCache"),updated);
        addToast("AI estimate complete — saved to cache");
      }catch(err){
    ...
    const loadFromHistory=(entry)=>{setF(p=>({...p,address:entry.address}));applyAiResult(entry.result);setAiFromCache(true);addToast("Loaded from history");};
    const historyEntries=Object.values(aiCache).filter(e=>e.date).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,10);
    ...
    return(<div><TopBar title="Deal Calculator"/>
      ...
      {aiResult&&<Card style={{marginBottom:18,borderColor:C.gold+"30"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div style={{fontSize:16,color:C.goldLight,fontWeight:500,fontFamily:FH}}>AI Property Estimate <span style={{fontSize:11,color:C.textMuted,fontWeight:400,fontFamily:FB}}>— verify independently</span></div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {aiFromCache&&<Badge color={C.blue}>Cached</Badge>}
            {aiFromCache&&<button onClick={()=>runAiEstimate(true)} style={{...btnOutline,padding:"4px 12px",fontSize:11}}>Refresh</button>}
          </div>
        </div>
        <div className="dash-two-col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
          <div>
      ...
      {(hasFMV||showManual)&&<><style>{`input[type=range].gold-slider{-webkit-appearance:none;appearance:none;width:100%;height:8px;border-radius:4px;background:rgba(255,255,255,0.06);outline:none;cursor:pointer;}input[type=range].gold-slider::-webkit-slider-runnable-track{height:8px;border-radius:4px;}input[type=range].gold-slider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:24px;height:24px;border-radius:50%;background:${C.gold};border:3px solid #0D0D0D;cursor:grab;box-shadow:0 2px 8px rgba(0,0,0,0.5);margin-top:-8px;}input[type=range].gold-slider::-webkit-slider-thumb:active{cursor:grabbing;transform:scale(1.2);}input[type=range].gold-slider::-moz-range-thumb{width:24px;height:24px;border-radius:50%;background:${C.gold};border:3px solid #0D0D0D;cursor:grab;box-shadow:0 2px 8px rgba(0,0,0,0.5);}input[type=range].gold-slider::-moz-range-track{height:8px;border-radius:4px;background:rgba(255,255,255,0.06);}`}</style>
      <div className="dash-two-col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
        <Card>
          ...
          <FormField label="Buyer Price" full><input style={inputStyle} type="number" value={f.buyerPrice} onChange={e=>setF(p=>({...p,buyerPrice:e.target.value}))} placeholder="What your buyer will pay you"/></FormField>
          <FormField label={`Offer % of FMV: ${f.offerPct}%`} full><input type="range" className="gold-slider" min="30" max="90" value={f.offerPct} onInput={e=>setF(p=>({...p,offerPct:Number(e.target.value)}))} onChange={e=>setF(p=>({...p,offerPct:Number(e.target.value)}))}/></FormField>
          <div style={{fontSize:11,color:C.textMuted,marginTop:8}}>You offer {f.offerPct}% of {fmt$(f.fmv)} to the seller = {fmt$(offerPrice)}</div>
        </Card>
        <Card>
          <div style={{fontSize:16,color:C.text,fontWeight:500,fontFamily:FH,marginBottom:14}}>Output</div>
          <div style={{display:"grid",gap:22}}>
            <div><div style={{fontSize:11,color:C.textSec,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>Recommended Offer <span style={{fontSize:9,color:C.textMuted,letterSpacing:0,textTransform:"none"}}>(what you pay seller)</span></div><div style={{fontSize:32,fontWeight:400,color:C.goldLight,fontFamily:FH}}>{fmt$(offerPrice)}</div></div>
            <div><div style={{fontSize:11,color:C.textSec,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>Your Profit <span style={{fontSize:9,color:C.textMuted,letterSpacing:0,textTransform:"none"}}>(buyer price − your offer)</span></div>{f.buyerPrice?<div style={{fontSize:32,fontWeight:400,color:actualProfit>0?C.green:C.red,fontFamily:FH}}>{fmt$(actualProfit)}</div>:<div style={{fontSize:16,color:C.textMuted,fontStyle:"italic"}}>Enter buyer price to calculate</div>}</div>
            {f.buyerPrice&&<div><div style={{fontSize:11,color:C.textSec,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>Buyer Savings vs FMV <span style={{fontSize:9,color:C.textMuted,letterSpacing:0,textTransform:"none"}}>(discount off market value)</span></div><div style={{fontSize:22,color:C.text}}>{fmt$(Math.abs(buyerSavings))} ({f.fmv?(Math.abs(buyerSavings)/Number(f.fmv)*100).toFixed(0):0}% below FMV)</div></div>}
      ...
      {/* Estimate History */}
      {historyEntries.length>0&&<Card style={{marginTop:18}}>
        <div style={{fontSize:16,color:C.text,fontWeight:500,fontFamily:FH,marginBottom:12}}>Recent Estimates</div>
        <div style={{maxHeight:240,overflowY:"auto"}}>{historyEntries.map((e,i)=><div key={i} onClick={()=>loadFromHistory(e)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 8px",borderBottom:`1px solid ${C.goldBorder}`,cursor:"pointer",borderRadius:6}} onMouseEnter={ev=>ev.currentTarget.style.background="rgba(212,175,55,0.04)"} onMouseLeave={ev=>ev.currentTarget.style.background="transparent"}>
          <div><div style={{fontSize:13,color:C.text}}>{e.address}</div><div style={{fontSize:11,color:C.textMuted}}>{e.result?.estimated_fmv_low&&e.result?.estimated_fmv_high?`FMV: ${fmt$(e.result.estimated_fmv_low)} – ${fmt$(e.result.estimated_fmv_high)}`:"—"}</div></div>
          <div style={{textAlign:"right"}}><div style={{fontSize:11,color:C.textMuted}}>{fmtDate(e.date?.split("T")[0])}</div><Badge color={e.result?.confidence==="high"?C.green:e.result?.confidence==="medium"?C.orange:C.red}>{e.result?.confidence||"?"}</Badge></div>
        </div>)}</div>
      </Card>}
...
export default function MonarchOS() {
  ...
  const AnalyzerPage = () => {
    ...
    return(<div><TopBar title="Deal Calculator"/>
      ...
      {/* Step 2: Deal Math (shows after AI fills FMV or manual entry) */}
      {(hasFMV||showManual)&&<><style>{`input[type=range].gold-slider{-webkit-appearance:none;appearance:none;width:100%;height:8px;border-radius:4px;background:linear-gradient(to right,${C.gold} 0%,${C.gold} var(--val),rgba(255,255,255,0.06) var(--val),rgba(255,255,255,0.06) 100%);outline:none;cursor:pointer;}input[type=range].gold-slider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:22px;height:22px;border-radius:50%;background:${C.gold};border:3px solid #0D0D0D;cursor:grab;box-shadow:0 2px 8px rgba(0,0,0,0.4);}input[type=range].gold-slider::-webkit-slider-thumb:active{cursor:grabbing;transform:scale(1.15);}input[type=range].gold-slider::-moz-range-thumb{width:22px;height:22px;border-radius:50%;background:${C.gold};border:3px solid #0D0D0D;cursor:grab;box-shadow:0 2px 8px rgba(0,0,0,0.4);}input[type=range].gold-slider::-moz-range-track{height:8px;border-radius:4px;background:rgba(255,255,255,0.06);}`}</style>
      <div className="dash-two-col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
        <Card>
          <div style={{fontSize:16,color:C.text,fontWeight:500,fontFamily:FH,marginBottom:14}}>Deal Details</div>
          <div style={formRow}><FormField label="Acres"><input style={inputStyle} type="number" value={f.acres} onChange={e=>setF(p=>({...p,acres:e.target.value}))}/></FormField><FormField label="Fair Market Value"><input style={inputStyle} type="number" value={f.fmv} onChange={e=>setF(p=>({...p,fmv:e.target.value}))}/></FormField></div>
          <FormField label="Buyer Price" full><input style={inputStyle} type="number" value={f.buyerPrice} onChange={e=>setF(p=>({...p,buyerPrice:e.target.value}))} placeholder="What your buyer will pay you"/></FormField>
          <FormField label={`Offer % of FMV: ${f.offerPct}%`} full><input type="range" className="gold-slider" min="30" max="90" value={f.offerPct} onChange={e=>setF(p=>({...p,offerPct:Number(e.target.value)}))} style={{"--val":`${((f.offerPct-30)/60)*100}%`}}/></FormField>
          <div style={{fontSize:11,color:C.textMuted,marginTop:8}}>You offer {f.offerPct}% of {fmt$(f.fmv)} to the seller = {fmt$(offerPrice)}</div>
        </Card>
        <Card>
          <div style={{fontSize:16,color:C.text,fontWeight:500,fontFamily:FH,marginBottom:14}}>Output</div>
