...
export default function MonarchOS() {
  ...
  const Sidebar = ({mobile}) => (
    <div style={{width:mobile?260:sW,minWidth:mobile?260:sW,background:C.bgSidebar,borderRight:`1px solid ${C.goldBorder}`,display:"flex",flexDirection:"column",height:"100vh",overflowY:"auto",position:mobile?"fixed":"sticky",top:0,zIndex:mobile?1000:10,...(mobile?{left:0}:{}),transition:"width 0.3s, min-width 0.3s"}}>
      ...
      {!sidebarCollapsed&&<div style={{padding:"10px 16px",position:"relative"}}>
        <input value={globalSearch} onChange={e=>{setGlobalSearch(e.target.value);setShowSearchResults(e.target.value.length>=2);}} onFocus={()=>{if(globalSearch.length>=2)setShowSearchResults(true);}} placeholder="🔍 Search..." style={{width:"100%",background:"rgba(255,255,255,0.04)",border:`1px solid ${C.goldBorder}`,borderRadius:10,padding:"8px 12px",fontSize:12,color:C.text,outline:"none",fontFamily:"inherit",boxSizing:"border-box"}}/>
        {showSearchResults&&globalSearch.length>=2&&(()=>{const q=globalSearch.toLowerCase();const sr=sellers.filter(s=>!s.archived&&((s.name||"").toLowerCase().includes(q)||(s.address||"").toLowerCase().includes(q)||(s.phone||"").includes(q))).slice(0,5);const dr=deals.filter(d=>(d.address||"").toLowerCase().includes(q)).slice(0,5);const br=buyers.filter(b=>(b.name||"").toLowerCase().includes(q)||(b.locations||"").toLowerCase().includes(q)).slice(0,5);const cr=calls.filter(c=>(c.discussed||"").toLowerCase().includes(q)||(c.notes||"").toLowerCase().includes(q)).slice(0,3);if(!sr.length&&!dr.length&&!br.length&&!cr.length)return<div style={{position:"fixed",left:250,top:140,zIndex:2000,background:C.bgCard,border:`1px solid ${C.goldBorder}`,borderRadius:14,padding:16,minWidth:280,boxShadow:"0 16px 40px rgba(0,0,0,0.4)"}}><p style={{color:C.textMuted,fontSize:12,margin:0}}>No results</p></div>;
        return<div style={{position:"fixed",left:250,top:140,zIndex:2000,background:C.bgCard,border:`1px solid ${C.goldBorder}`,borderRadius:14,padding:12,minWidth:280,maxHeight:400,overflowY:"auto",boxShadow:"0 16px 40px rgba(0,0,0,0.4)"}} onClick={e=>e.stopPropagation()}>
          {sr.length>0&&<><div style={{fontSize:10,color:C.textMuted,letterSpacing:1,padding:"4px 0"}}>SELLERS</div>{sr.map(s=><div key={s.id} onClick={()=>{nav("sellers");setGlobalSearch("");setShowSearchResults(false);}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 6px",cursor:"pointer",borderBottom:`1px solid ${C.goldBorder}`,fontSize:12}}><span style={{color:C.text}}>{s.name||s.address}</span><Badge color={STATUS_COLORS[s.status]}>{s.status}</Badge></div>)}</>}
          {dr.length>0&&<><div style={{fontSize:10,color:C.textMuted,letterSpacing:1,padding:"8px 0 4px"}}>DEALS</div>{dr.map(d=><div key={d.id} onClick={()=>{nav("pipeline");setGlobalSearch("");setShowSearchResults(false);}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 6px",cursor:"pointer",borderBottom:`1px solid ${C.goldBorder}`,fontSize:12}}><span style={{color:C.text}}>{d.address||"—"}</span><Badge color={STATUS_COLORS[d.status]}>{d.status}</Badge></div>)}</>}
          {br.length>0&&<><div style={{fontSize:10,color:C.textMuted,letterSpacing:1,padding:"8px 0 4px"}}>BUYERS</div>{br.map(b=><div key={b.id} onClick={()=>{nav("buyers");setGlobalSearch("");setShowSearchResults(false);}} style={{padding:"8px 6px",cursor:"pointer",borderBottom:`1px solid ${C.goldBorder}`,fontSize:12,color:C.text}}>{b.name}</div>)}</>}
          {cr.length>0&&<><div style={{fontSize:10,color:C.textMuted,letterSpacing:1,padding:"8px 0 4px"}}>CALLS</div>{cr.map(c=><div key={c.id} onClick={()=>{nav("calls");setGlobalSearch("");setShowSearchResults(false);}} style={{padding:"8px 6px",cursor:"pointer",borderBottom:`1px solid ${C.goldBorder}`,fontSize:12,color:C.textSec}}>{(c.discussed||c.notes||"—").slice(0,60)}</div>)}</>}
        </div>;})()}
      </div>}
      {sidebarCollapsed&&<div style={{padding:"10px 0",textAlign:"center",cursor:"pointer",color:C.textMuted,fontSize:16}} onClick={()=>setSidebarCollapsed(false)}>🔍</div>}
  ...
  const CallsPage = () => {
    ...
    const exitCallMode=()=>{setCallMode(false);setCmStep("pick");setCmSeller(null);setCmDevice(null);setCmStage("opening");setCmLastStatus("");setCmSearch("");};
    ...
    const cmEndCall=()=>{setForm({leadId:cmSeller?.id||"",buyerId:"",date:todayStr(),time:new Date().toLocaleTimeString("en-US",{hour12:false,hour:"2-digit",minute:"2-digit"}),result:cmLastStatus||"Answered",notes:"",followUp:"",discussed:""});setModal(true);exitCallMode();};
      ...
      return(<div>
        ...
        <div style={cmDevice==="other"?{display:"grid",gridTemplateColumns:"320px 1fr",gap:18}:{maxWidth:520,margin:"0 auto"}}>
          <div>
            ...
            {cmDevice==="other"&&<Card><div style={{fontSize:12,color:C.textMuted,letterSpacing:1,marginBottom:8}}>AI COACH</div><p style={{fontSize:13,color:C.textSec}}>Live transcription coming in Part B update.</p></Card>}
          </div>
          <div>
    ...
    return(<div><TopBar title="Call Tracker"/><div style={{display:"flex",justifyContent:"flex-end",gap:10,marginBottom:18}}>
      <button style={{...btnStyle,background:`linear-gradient(135deg,${C.gold},${C.goldDim})`}} onClick={()=>{setCallMode(true);setCmStep("pick");}}>📞 Call Mode</button>
