...
export default function MonarchOS() {
    ...
    return(<div><TopBar title="Analytics"/>
      ...
      {tab==="velocity"&&<div>
        ...
        <Card style={{marginBottom:14}}><div style={{fontSize:16,color:C.text,fontWeight:500,fontFamily:FH,marginBottom:14}}>Deal Conversion Funnel</div>
          <svg viewBox="0 0 400 130" style={{width:"100%",maxWidth:460}}>{DEAL_STATUSES.map((s,i)=>{const w=320-i*45;const x=(400-w)/2;const y=i*25;const prevC=i>0?sc[DEAL_STATUSES[i-1]]:0;const convR=i===0?"":prevC?(sc[s]/prevC*100).toFixed(0)+"%":"0%";return<g key={s}><rect x={x} y={y} width={w} height={20} rx={4} fill={STATUS_COLORS[s]} opacity={0.35}/><text x={200} y={y+14} textAnchor="middle" fill={C.text} fontSize="9" fontWeight="500" opacity="0.8">{s}: {sc[s]}{convR&&` · ${convR}`}</text></g>;})}</svg>
        </Card>
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:C.bg,color:C.text,fontFamily:FB}}>
      ...
      {!sidebarCollapsed&&<div className="m-sidebar" style={{position:"fixed",top:profile?140:90,left:16,width:sW-32,zIndex:15}}>
